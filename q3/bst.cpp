...
public:
    ...
    Node* insertRecursively(Node* node, int value) {
        ...
        if (value < node->data)
            ...
            node->right = insertRecursively(node->right, value); // Go to right subtree

        return node;
    }

    // Helper to call recursive insert with root
    void insertRecursively(int value) {
    ...
    bool searchRecursively(int value) {
...
int main() {
    ...
    cout << "Search 7 Iteratively: " << (tree.searchIteratively(7) ? "Found" : "Not Found") << "\n";
    cout << "Search 100 Recursively: " << (tree.searchRecursively(100) ? "Found" : "Not Found") << "\n";
